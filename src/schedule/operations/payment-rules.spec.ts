...
describe('Payment Rules', () => {
  ...
  const createBaseRule = (overrides: Partial<PaymentRule> = {}): PaymentRule => ({
    id: '1',
    venueId,
    spaceIds: [spaceId],
    priority: 1,
  ...
  describe('Base Rate Rules', () => {
    ...
    it('should round up partial periods', () => {
      ...
      const result = runPaymentRules(rules, baseDate, ninetyMinutesLater, spaceId);
      expect(result.requiresPayment).toBe(true);
  ...
  describe('Time and Day Restrictions', () => {
    it('should only apply rules within specified time range', () => {
      ...
      const result = runPaymentRules(rules, outsideTimeRange, twoHoursLater, spaceId);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle decimal precision in complex calculations', () => {
      ...
      const result = runPaymentRules(rules, baseDate, thirtyMinutesLater, spaceId);
      expect(result.requiresPayment).toBe(true);
    ...
    it('should handle rules with global space applicability', () => {
      ...
      const result = runPaymentRules(rules, baseDate, oneHourLater, 'any-space-id');
      expect(result.requiresPayment).toBe(true);
      expect(result.totalCost).toBe(10);
    });
  });
