...
export const CalendarHeader: FC<{ venue: WeekViewCalendarProps['venue'] }> = ({ venue }) => {
  ...
  >
    <div className="flex px-4 py-2 bg-gray-200 gap-2 items-center">
      ...
      <ButtonGroup
        items={[
          { label: <ChevronLeftIcon className="size-6" />, onClick: () => setSelectedDate(addDays(selectedDate, -1)) },
          { label: <ChevronRightIcon className="size-6" />, onClick: () => setSelectedDate(addDays(selectedDate, 1)) },
        ]}
      />
    ...
    <div className="z-30 flex-none bg-white shadow sm:pr-8">
      ...
      >
        ...
        {venue.spaces.map((space, index) => (
          ...
          >
            <span className="flex items-baseline font-medium text-gray-900">{space.name}</span>
          </div>
        ))}
      </div>
    </div>
